...
class Workspace {
 public:
  ...
  // std::shared_ptr<SparsePuller>& GetSparsePuller() { return sparse_puller_; }
  ...
  // Only used for small model creation(Mainly Text Pb DNN Model), such as:
  // Testing model NOTE: not used in production env.
  ...
  DataType input_data_type(const std::string& input_name) const {
    ...
    CHECK(iter != input_data_type_map_.end(),
          "input_name: %s data_type not defined", input_name.c_str());
    return iter->second;
  }
  ...
  DataType output_data_type(const std::string& output_name) const {
    ...
    CHECK(iter != output_data_type_map_.end(),
          "output_name: %s data_type not defined", output_name.c_str());
    return iter->second;
  }
  ...
  const ValueInfo& input_info(const std::string& input_name) const {
    ...
    CHECK_TRUE(iter != input_info_map_.end(), "input_name=", input_name,
               " is not input name");
    return iter->second;
  }
 ...
 protected:
  ...
  Blob* GetConstantBlob(const std::string& name) {
    ...
    CHECK(iter != constant_fill_blob_map_.end(), "blob %s not existed",
          name.c_str());
    return iter->second;
  }
